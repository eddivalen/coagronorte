...
-- Mon 20 Mar 2017 01:16:48 PM VET
...
CREATE TABLE IF NOT EXISTS `coagronorte`.`usuarios` (
  ...
  `reset_password_token` BLOB NULL DEFAULT NULL COMMENT 'Token  generado para validar el reinicio de contraseña',
  `timestamp` TIMESTAMP NULL DEFAULT NULL,
  `confirmacion` INT(11) NULL DEFAULT '0' COMMENT 'Campo usado para determinar si el usuario ya confirmo su cuenta a través de su correo electronico.',
  `confirmation_token` LONGTEXT NULL DEFAULT NULL COMMENT 'Token generado para confirmar la cuenta del usuario',
  PRIMARY KEY (`cedula`),
...
CREATE TABLE IF NOT EXISTS `coagronorte`.`visitas` (
  ...
  `presencia_agricultor` BINARY(1) NULL DEFAULT NULL COMMENT 'hubo presencia del agricultor en la visita 1:Si 0:No',
  `registro_ausencia` TEXT NULL DEFAULT NULL COMMENT 'justificacion de la ausencia del agricultor',
  `validado` BINARY(1) NULL DEFAULT NULL COMMENT 'indica si el agricultor ya valido el informe de la visita 1:Si 0:No',
  `valoracion` INT(11) NULL DEFAULT NULL COMMENT 'valoracion otorgada por agricultor al agronomo durante la visita 1 - 5',
  `opinion` TEXT NULL DEFAULT NULL COMMENT 'opinion emitida por agricultor a la visita',
  `agronomo` VARCHAR(50) NULL DEFAULT NULL COMMENT 'clave foranea con tabla usuarios para identificar que agronomo realizo la visita',
...
-- -----------------------------------------------------
-- Table `coagronorte`.`failed_jobs`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `coagronorte`.`failed_jobs` (
...
-- -----------------------------------------------------
-- Table `coagronorte`.`jobs`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `coagronorte`.`jobs` (
  ...
  PRIMARY KEY (`id`),
  INDEX `jobs_queue_reserved_reserved_at_index` (`queue` ASC, `reserved` ASC, `reserved_at` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `coagronorte`.`productos` (
  ...
  PRIMARY KEY (`codigo`),
  INDEX `fk_productos_unidades1_idx` (`codigo_unidad` ASC),
  CONSTRAINT `fk_productos_unidades1`
    FOREIGN KEY (`codigo_unidad`)
    REFERENCES `coagronorte`.`unidades` (`codigo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
AUTO_INCREMENT = 3
